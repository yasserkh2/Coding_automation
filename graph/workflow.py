...
from .nodes import EnhanceProjectNode, NewProjectNode, ProjectRouterNode
...
def create_coding_graph(speaker: CodexSpeaker | None = None, config_path: Path | None = None):
    ...
    ``enhance_project``. Both implementation nodes call the same
    ``CodexSpeaker`` interface so the graph is easy to test with fakes.
    """
    ...
    graph.add_node(new_project, NewProjectNode(codex_speaker))
    ...
    graph.add_edge(new_project, END)
